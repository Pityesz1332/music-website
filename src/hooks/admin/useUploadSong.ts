...
import { uploadFile } from "../../services/swarm";
...
export const useUploadSong = (onSave: (song: any) => void) => {
    ...
    const handleUpload = async() => {
        ...
        try {
            // audio uploading
            const audioHash = await uploadFile(audioFile, (progress) => setProgress(progress * 0.5));
            let coverHash = "";

            if (coverFile) {
                coverHash = await uploadFile(coverFile, (progress) => setProgress(50 + progress * 0.5));
            }
